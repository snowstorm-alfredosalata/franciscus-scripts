...
file without calling Claude again.

Usage:
    python ai_process/process.py books/1Cel.md --translate it
    python ai_process/process.py books/1Cel.md --annotate attributes.toml
    python ai_process/process.py books/1Cel.md --translate it --annotate attributes.toml
...
def call_claude(system_prompt: str, user_prompt: str, schema: dict) -> dict:
    """Invoke claude CLI and return parsed JSON."""
    cmd = [
        "claude",
        "-p",
        "--output-format", "json",
...
def main():
    parser = argparse.ArgumentParser(
        description="Process Franciscus source texts with Claude for translation/annotation"
    )
    ...
    if args.compile:
        ...
        for i, block in enumerate(blocks):
            ...
            res = call_claude(system_prompt, user_prompt, schema)
            results.append(res)
